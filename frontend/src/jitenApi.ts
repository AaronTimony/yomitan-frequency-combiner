...
export const MEDIA_TYPES: Array<{ id: number; label: string }> = [
  { id: 1, label: "Anime" },
  { id: 2, label: "Drama" },
  { id: 3, label: "Movie" },
  { id: 4, label: "Novel" },
...
const MEDIA_TYPE_LABELS: Record<number, string> = Object.fromEntries(
  MEDIA_TYPES.map(({ id, label }) => [id, label]),
);
...
export async function fetchDecks(query = "", page = 1, mediaType?: number): Promise<FetchDecksResult> {
  const params = new URLSearchParams();
  if (query) params.set("titleFilter", query);
  if (mediaType !== undefined) params.set("mediaType", String(mediaType));
 ...
 */
export async function fetchDeckYomitanZip(deck: JitenDeck, attempts = 5): Promise<Blob> {
