...
function setupGenreCart() {
  ...
  `;
    ...
    function render() {
        ...
        if (entries.size === 0)
            statusEl.textContent = "Add at least 2 lists to merge.";
        else if (entries.size === 1)
            statusEl.textContent = "Add 1 more list to merge.";
        else
            statusEl.textContent = `${entries.size} lists ready to merge.`;
    }
    ...
    mergeBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            for (let i = 0; i < selected.length; i++) {
                ...
                if (!res.ok)
                    throw new Error(`HTTP ${res.status} — could not fetch ${selected[i].name} from ${DICT_BASE_URL}`);
...
function genreFileSlug(genre) {
    return genre.replace(/\s+/g, "_");
}
function populateFeaturedGrids() {
...
function populateAllGenresSections() {
