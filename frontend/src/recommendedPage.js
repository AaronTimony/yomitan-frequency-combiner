...
async function loadGenreRow(row) {
    const sourcesUrl = row.dataset.genreUrl;
    const cacheKey = `sources-v1-${sourcesUrl}`;
    const wordsEl = row.querySelector('[data-stat="words"]');
    const decksEl = row.querySelector('[data-stat="decks"]');
    const addBtn = row.querySelector("[data-add-genre]");
    try {
        let data;
        const cached = sessionStorage.getItem(cacheKey);
        if (cached) {
...
function setupGenreCart(listId) {
    const list = document.getElementById(listId);
    if (!list)
        return;
    const rowsWrap = list.querySelector("#genre-rows");
    if (!rowsWrap)
        return;
  ...
  `;
    ...
    const totalWordsEl = cart.querySelector("[data-total-words]");
    const totalDecksEl = cart.querySelector("[data-total-decks]");
    const nameInput = cart.querySelector("[data-name]");
    const mergeBtn = cart.querySelector("[data-merge]");
    const statusEl = cart.querySelector("[data-status]");
    const clearBtn = cart.querySelector("[data-clear]");
    const entries = new Map();
    function setRowAdded(row, added) {
        const btn = row.querySelector("[data-add-genre]");
        if (!btn)
            return;
    ...
    function render() {
        ...
        if (entries.size === 0)
            statusEl.textContent = "Add at least 2 genres to merge.";
        else if (entries.size === 1)
            statusEl.textContent = "Add 1 more genre to merge.";
        else
            statusEl.textContent = `${entries.size} genres ready to merge.`;
    }
    rowsWrap.addEventListener("click", (ev) => {
        const btn = ev.target.closest("[data-add-genre]");
        if (!btn || btn.disabled)
            return;
        const row = btn.closest("[data-zip-url]");
        if (!row)
            return;
        ...
        if (entries.has(key)) {
            ...
            entries.set(key, {
                zipUrl: key,
                name: row.querySelector("[data-genre-name]")?.textContent?.trim() ?? "Genre",
    ...
    mergeBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            for (let i = 0; i < selected.length; i++) {
                ...
                if (!res.ok)
                    throw new Error(`HTTP ${res.status} — is ${selected[i].name} in /dicts/?`);
    ...
    document.querySelectorAll("[data-media-toggle]").forEach((btn) => {
        ...
        btn.addEventListener("click", () => {
            ...
            if (isOpen) {
                ...
                if (label)
                    label.textContent = "Browse genres";
                ...
                if (label)
                    label.textContent = "Hide genres";
                chevron?.classList.add("rotate-180");
                list.querySelectorAll("[data-genre-url]").forEach((row) => {
...
export function setupRecommendedPage() {
    document.querySelectorAll("article[data-sources-url]").forEach(attachSourcesPanel);
    setupMediaDropdowns();
