import { fetchDecks, deckTitle, fetchDeckYomitanZip, type JitenDeck } from "./jitenApi";
import { mergeJitenDecks, downloadBlob, type DictSource } from "./combiner";
...
export interface MatchedDeck {
  anilistDisplay: string;
  anilistRomaji: string;
  deck: JitenDeck;
}
...
async function fetchAniListPage(genre: string, page: number, attempt = 0): Promise<AniListPage> {
  try {
...
async function searchJiten(title: string, attempt = 0): Promise<JitenDeck | null> {
  try {
...
async function downloadWithRetry(deck: JitenDeck, attempt = 0): Promise<Blob> {
  try {
...
function makeMatchRow(m: MatchedDeck): HTMLElement {
...
function renderResultsHeader(results: HTMLElement): void {
  results.innerHTML = `
...
export function setupGenrePage(section: HTMLElement): void {
  section.innerHTML = `
  ...
  const btn = document.getElementById("genre-run-btn") as HTMLButtonElement;
  const resumeBtn = document.getElementById("genre-resume-btn") as HTMLButtonElement;
  ...
  const mergeBtn = document.getElementById("genre-merge-btn") as HTMLButtonElement;
  const dictTitleInput = document.getElementById("genre-dict-title") as HTMLInputElement;

  // Restore state if the user navigated away and came back
  if (savedState) {
    input.value = savedState.genre;
    renderResultsHeader(results);
    for (const m of savedState.matched) results.appendChild(makeMatchRow(m));
  ...
  mergeBtn.addEventListener("click", async () => {
    if (!savedState || savedState.matched.length === 0) return;
    ...
    try {
      ...
      for (let i = 0; i < decks.length; i++) {
        ...
        if (i < decks.length - 1) await sleep(500);
      }
      mergeBtn.textContent = "Merging…";
      const files = blobs.map((blob, i) => new File([blob], `deck_${decks[i].deckId}.zip`));
      const sources: DictSource[] = savedState.matched.map((m) => ({
      ...
      downloadBlob(
        new Blob([sourcesJson], { type: "application/json" }),
        `${safeFilename(title)}_sources.json`,
      );
    } catch (err) {
  ...
  input.addEventListener("keydown", (e) => { if (e.key === "Enter") startRun(false); });

  async function startRun(resume: boolean): Promise<void> {
